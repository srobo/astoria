...
def test_metadata_fields() -> None:
    ...
    metadata = Metadata(
        ...
        game_timeout=120,
        wifi_enabled=False,
        client_wifi_ssid="arena-robots",
        client_wifi_psk="password123",
        client_wifi_region="GB",
    ...
    assert metadata.client_wifi_ssid == "arena-robots"
    assert metadata.client_wifi_psk == "password123"
    assert metadata.client_wifi_region == "GB"
    ...
    assert metadata.json() == '{"arena": "B", "zone": 12, "mode": "COMP", "marker_offset": 40, "game_timeout": 120, "wifi_enabled": false, "client_wifi_ssid": "arena-robots", "client_wifi_psk": "password123", "client_wifi_region": "GB", "astoria_version": "0.0.0", "kernel_version": "5.0.0", "arch": "x64", "python_version": "3", "libc_ver": "2.0", "os_name": "Student Robotics OS", "os_pretty_name": "Student Robotics OS 2023.0.0", "os_version": "2023.0.0", "usercode_entrypoint": "robot.py", "wifi_ssid": "robot", "wifi_psk": "bees", "wifi_region": "GB"}'  # noqa: E501
...
def test_metadata_fields_default() -> None:
    ...
    assert metadata.client_wifi_ssid is None
    assert metadata.client_wifi_psk is None
    assert metadata.client_wifi_region is None
    assert metadata.astoria_version == "0.0.0"
    assert metadata.kernel_version == "5.0.0"
    assert metadata.arch == "x64"
    assert metadata.python_version == "3"
